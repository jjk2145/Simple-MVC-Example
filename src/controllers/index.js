...
var lastAddedDog = new Dog(defaultDataDog);
...
var readCat = function(req, res) {
    ...
    Cat.findByNameCat(name1, callback);
};
...
//controller functions in Express receive the full HTTP request 
//and a pre-filled out response object to send
var getNameCat = function(req, res) {
...
var getNameDog = function(req, res) {
    //res.json returns json to the page. Since this sends back the data through HTTP
    //you can't send any more data to this user until the next response
...
var setNameCat = function(req, res) {
    ...
    newCat.save(function(err) {
        ...
        //set the lastAddedCat cat to our newest cat object. This way we can update it dynamically
        lastAddedCat = newCat;
...
var setNameDog = function(req, res) {
    
    //check if the required fields exist
    //normally you would also perform validation to know if the data they sent you was real 
    if(!req.body.firstname || !req.body.lastname || !req.body.breed || !reg.body.age) {
        //if not respond with a 400 error (either through json or a web page depending on the client dev)
...
//controller functions in Express receive the full HTTP request 
//and a pre-filled out response object to send
var searchNameCat = function(req,res) {
    ...
    Cat.findByNameCat(req.query.name, function(err, doc) {
        //errs, handle them
        if(err) {
            return res.json({err:err}); //if error, return it            
        }
...
var searchNameDog = function(req,res) {
	...
	Dog.findByNameDog(req.query.name, function(err, doc) {
        //errs, handle them
        if(err) {
            return res.json({err:err}); //if error, return it            
        }
...
var updateLastCat = function(req, res) {
    ...
    lastAddedCat.save(function(err) {
        ...
        return res.json({name:lastAddedCat.name, beds: lastAddedCat.bedsOwned});
    });
};
...
module.exports = {
    ...
    page2: hostPage2,
    page3: hostPage3, 
    getNameCat: getNameCat,
	getNameDog: getNameDog,
    setNameCat: setNameCat,
	setNameDog: setNameDog,
    updateLastCat: updateLastCat,
	updateLastDog: updateLastDog,
    searchNameCat: searchNameCat,
	searchNameDog: searchNameDog,
    notFound: notFound
};